...
class SettingsViewModel(application: Application): AndroidViewModel(application) {
    ...
    val deviceDamping: Flow<Double> = dataStore.data.map { preferences ->
        preferences[SettingsPreferences.DEVICE_DAMPING] ?: AppConstants.SETTINGS_DEFAULT_DEVICE_DAMPING
    }

    val signalDetectionPeriod: Flow<Double> = dataStore.data.map { preferences ->
        preferences[SettingsPreferences.SIGNAL_DETECTION_PERIOD] ?: AppConstants.SETTINGS_DEFAULT_SIGNAL_DETECTION_PERIOD
    }

    val enableNotifications: Flow<Boolean> = dataStore.data.map { preferences ->
        preferences[SettingsPreferences.ENABLE_NOTIFICATIONS] ?: AppConstants.SETTINGS_DEFAULT_ENABLE_NOTIFICATIONS
    }

    val angerDetectionTime: Flow<Double> = dataStore.data.map { preferences ->
        preferences[SettingsPreferences.ANGER_DETECTION_TIME] ?: AppConstants.SETTINGS_DEFAULT_ANGER_DETECTION_TIME
    }
